...
select c.relname table_, a.attname column_, c.relhasindex hasindex, c.relpages pages, c.reltuples rows_,
		round(s.stanullfrac::numeric, 2) null_percent, round((s.stadistinct/(c.reltuples*(1-s.stanullfrac)))::numeric, 2) dist_percent,
...
where c.relkind in ('r', 'm') --	r=ordinary table, i=index, S=sequence, v=view, m=materialized view, c=composite type, t=TOAST table, f=foreign table
